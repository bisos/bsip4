...
function G_postParamHook {
    ...
    # NOTYET -- De-Activated for now -- Should be reactivated again
    # bisosCurrentsGet
}
...
$( examplesSeperatorChapter "Sited BPO Developer Git Credentials Activate" )
...
$( examplesSeperatorChapter "Unsited BRO Developer Git Credentials Activate" )
${G_myName} ${extraInfo} -i bisosDevBxo_fullSetup bro  # PRIMARY -- activate bisosDevBxoId plus actuate it
${G_myName} ${extraInfo} -i bisosDevBxo_activate bro   # activate bisosDevBxoId
...
bx-gitReposBases -v 20 --baseDir="/bisos/git/auth/bxRepos" --pbdName="bxReposRoot" --vcMode="auth" --gitLabel="mb2_github"  -i pbdUpdate all
...
_EOF_
    ...
    lpDo vis_bisosDevBxo_activate ${bxoType}

    lpDo vis_bisosDevBxo_actuate ${bxoType}
...
_EOF_
    ...
    if [ "${bxoType}" == "bpo" ] ; then
        ...
        echo "${bisosDevBxoHome} -- is in place, clonning skipped"
        # NOTYET, Clone if needed
    else
        EH_oops ""
        lpReturn
    fi
...
_EOF_
    ...
    if [ $# -eq 0 ] ; then
        ...
        if [ "${bxoType}" == "bpo" ] ; then
            ...
            # auth clone using bisosDev credentials
            # switch to auth based bxRepos
            lpDo echo ${bisosDevBxoHome}/sys/bin/broSysSetup.sh ${G_commandPrefs} \
               -i developerMode

...
while getopts e:a:c:T:s:i:p:l:u?n:vfh c
do
    case $c in
    T)
        case "$OPTARG" in
...
G_commandOptions="${G_checkCmndOption} ${G_runModeCmndOption} ${G_verboseCmndOption} ${G_humanCmndOption} ${G_forceModeCmndOption}"
...
function targetSameAsFamily {
    typeset tmpFile=/tmp/targetSameAsFamily.$$

    #$1 -- Subject to be faked.
    #$2 -- Familyentry
...
function subjectValidatePrepareOld {

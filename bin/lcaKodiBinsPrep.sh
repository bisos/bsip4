...
####+BEGIN: bx:dblock:lsip:binsprep:apt :module "kodi"
_CommentBegin_
*      ======[[elisp:(org-cycle)][Fold]]====== Apt-Pkg: kodi
_CommentEnd_
item_kodi () { distFamilyGenerationHookRun binsPrep_kodi; }

binsPrep_kodi_DEFAULT_DEFAULT () { binsPrepAptPkgNameSet "kodi"; }
...
vis_repositoryAdd () {
    opDo sudo add-apt-repository ppa:team-xbmc/ppa
    opDo sudo apt-get update
}

vis_repositoryRemove () {
    opDo sudo add-apt-repository --remove ppa:team-xbmc/ppa
    opDo sudo apt-get update
}


item_kodi_stableAddRepo () {
  distFamilyGenerationHookRun binsPrep_kodi_stableAddRepo
}

binsPrep_kodi_stableAddRepo_DEFAULT_DEFAULT () {
    mmaThisPkgName="kodi_stableAddRepo"
...
item_kodi_addOns () {
  distFamilyGenerationHookRun binsPrep_kodi_addOns
}

binsPrep_kodi_addOns_DEFAULT_DEFAULT () {
    mmaThisPkgName="kodi_addOns"
    mmaPkgDebianName="${mmaThisPkgName}"
    mmaPkgDebianMethod="custom"  #  or "apt" no need for customInstallScript but with binsPrep_installPostHook

...
IimBriefDescription="Manage BSOs (Bx Static Obj) -- Copied from bpoGithubManage.sh -- Needs to be redone."
...
${G_myName} ${extraInfo} -p privacy="${priv}" -p bpoId="${oneBxoId}" -i bxoAcctCreate
...
${G_myName} ${extraInfo} -p privacy="${priv}" -p bpoId="${oneBxoId}" -i fullConstruct # Creats Acct & git clones in ${oneBxoHome}
_EOF_
}
...
_EOF_
    ...
    local bxeDesc="$(vis_bxoConstructBaseDir_obtain ${privacy})/${bpoId}/rbxe/bxeDesc"
...
_EOF_
    ...
    for eachRepo in ${reposList} ; do
        lpDo git clone git://github.com/bxObjects/${bpoId}.${eachRepo}.git "${gitCloneDest}/${eachRepo}"
    done

...
${G_myName} ${extraInfo} -i genPipreqs                           # Regenerate ./requirements.txt via pipreqs-plus.cs (handles .cs/.pcs/.mcs + bisos.* namespace expansion)
_EOF_
...
tree and ./bin. Thin wrapper around the =pipreqs-plus.cs= command from
the =bisos.pipreqs-plus= pip package, which handles:
  - Detecting Python source files with non-.py extensions (.cs, .pcs,
    .mcs) via shebang and extraExts.
  - Augmenting pipreqs output with two-level namespace-package
    submodule names (bisos.b, bisos.common, ...) that pipreqs
    collapses down to bare "bisos".

Requires: pipreqs and pipreqs-plus.cs on PATH.
...
_EOF_
    ...
    if ! command -v pipreqs-plus.cs > /dev/null 2>&1 ; then
        EH_problem "pipreqs-plus.cs not found on PATH; install with: pipx install bisos.pipreqs-plus"
        lpReturn 1
    fi
    ...
    lpDo pipreqs-plus.cs -i genRequirements \
        --srcDir="${pkgSrcTree}" \
        --binDir="./bin" \

...
_EOF_
    ...
    #NETWORK
    opDoComplain vis_netL2Interface
...
_EOF_
	...
	ANT_raw "${originalFileDotDist} Already Exists -- dateTagging it."
	originalFileDotDist="${originalFileDotDist}"."${dateTag}"
    fi
...
_EOF_
    ...
    opDo vis_fileDotDistKeep /etc/hostname
	...
	Server)
	    ...
	    opDo vis_fileDotDistKeep /etc/motd 
...
_EOF_
    ...
    if [ "${opRunHostMode}" != "unnet" ] ; then
	opDo vis_fileDotDistKeep "${hostsFileName}"

	if [ -z "${opRunDomainName}" ] ; then
	    opRunDomainName="by-star.net"
	fi

	opDo eval "${opBinBase}/opNetNameServices.sh  -p domainName=${opRunDomainName} -p clusterName=${opRunClusterName} -i hostTable mini > ${hostsFileName}"

	chmod 444 ${hostsFileName}
...
itemPre_ipMulti () {
  iv_ipHost_domainName="neda.com"

  function iv_extraInterfacesFunction {
    opNetCfg_paramsGet ${opRunClusterName} ${opRunHostName}

    if [[ "${opRunOsType}_" == "Linux_" ]] ; then
      cat  << _EOF_
# /etc/network/interfaces -- configuration file for ifup(8), ifdown(8)
# The following lines are generated by OSMT. All manual modifications will be
# lost on the next OSMT run.
auto lo
iface lo inet loopback

auto eth0
iface eth0 inet static
      address ${opNetCfg_ipAddr}
      netmask ${opNetCfg_netmask}
      network ${opNetCfg_networkAddr}
      gateway ${opNetCfg_defaultRoute}
      down /bin/sh /etc/network/control/mmaL3FirewallInit.sh  stop || exit 0
      down /bin/sh /etc/network/control/mmaL3ExtraRoutesInit.sh  stop || exit 0
      up   /bin/sh /etc/network/control/mmaL3FirewallInit.sh  start || exit 0
      up   /bin/sh /etc/network/control/mmaL3ExtraRoutesInit.sh  start || exit 0

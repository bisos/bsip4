...
typeset RcsId="$Id: bueAcctCurrent.sh,v 1.1.1.1 2016-06-08 23:49:51 lsipusr Exp $"

# /opt/public/osmt/bin/seedActions.bash
...
NB: lueBystarAcct.sh  is intended to replace lueBystarUser.sh 
...
distFamilyIsSetAssert  || return $?

debPkgsBaseDir="`distFamilyGenerationBaseDir`/debPkgs" || return $?
#debPkgsBaseDir="/opt/public/osmt/distBase/${cononDistFamily}/${cononDistGeneration}/debPkgs"

distRepositorySrcFile=${debPkgsBaseDir}/distRepositorySrc

FN_dirCreatePathIfNotThere ${debPkgsBaseDir}
...
vis_examples () {
  ...
  typeset oneUser=lsipusr 


  #typeset doLibExamples=`doLibExamplesOutput ${G_myName}`
 cat  << _EOF_
EXAMPLES:
${G_myName} ${extraInfo} -i seedCommonFeatures
--- INFORMATION ---
...
${G_myName} ${uidInfo} ${extraInfo} -p bystarUid=${oneBystarUid} -p user=lsipusr -p developer=lsipusr -i userServiceFullUpdate
--- ASSOCIATE BUA (ByStar User Account)  TO BSA (ByStar SERVICE Account) ---
...
${G_myName} ${uidInfo} ${extraInfo} -p bystarUid=${oneBystarUid} -p user=lsipusr -i userServiceAssociateCurrent # Do the symlinks
--- Current Updates ---
...
${G_myName} ${extraInfo} -p bystarUid=ea-59043 -p user=lsipusr -i offlineimapUpdate
...
${G_myName} ${extraInfo} -p bystarUid=ea-59043 -p user=lsipusr -i authInfoUpdate
...
${G_myName} ${extraInfo} -p bystarUid=ea-59043 -p user=lsipusr -i eoeUpdateOne
${G_myName} ${extraInfo} -p bystarUid=${oneBystarUid} -p user=${oneUser} -i eoeStdoutOne
---
${G_myName} ${extraInfo} -p bystarUid=${oneBystarUid} -p user=lsipusr -i bleeAcctMainStdout
${G_myName} ${extraInfo} -p bystarUid=${oneBystarUid} -p user=lsipusr -i bleeAcctMainUpdate
_EOF_
}
...
#
  ...
  cat <<  _EOF_ > ${userHome}/lisp/bystar-all.el
;; 
;; bystar-all.el
;; 
...
_EOF_
  ...
  opDo ls -l  ${userHome}/lisp/bystar-all.el
}

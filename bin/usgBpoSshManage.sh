...
${G_myName} ${examplesInfo} -p usg=${oneUsg} -p bpoId=${oneBxoId} -i usgBxoFullUpdate
${G_myName} ${examplesInfo} -p usg=${oneUsg} -p bpoId=${oneBxoId} -i usgBxoFullDelete
...
${G_myName} ${examplesInfo} -p bpoId=${oneBxoId} -i bxoSshKeyVerify
${G_myName} ${examplesInfo} -p bpoId=${oneBxoId} -i bxoSshKeyUpdate
$( examplesSeperatorSection "USG BxO Ssh Invoker Keys Install" )
${G_myName} ${examplesInfo} -p usg=${oneUsg} -p bpoId=${oneBxoId} -i usgAcctBxoCredentialsUpdate bxoPriv
${G_myName} ${examplesInfo} -p usg=${oneUsg} -p bpoId=${oneBxoId} -p bxosBase=${oneBxosBase} -i usgAcctBxoCredentialsUpdate bxoPriv
${G_myName} ${examplesInfo} -p usg=${oneUsg} -p bpoId=${oneBxoId} -i usgAcctBxoCredentialsDelete bxoPriv
...
${G_myName} -p bpoId=${oneBxoId} -i bxoConfigSegStdout bxoPriv ${site_gitServerName}
${G_myName} ${extraInfo} -p bpoId=${oneBxoId} -i bxoConfigSegStdout bxoPriv ${site_gitServerName}  # Verbose
${G_myName} ${extraInfo} -p bpoId=${oneBxoId} -i bxoConfigSegFileName bxoPriv ${site_gitServerName}
${G_myName} ${extraInfo} -p bpoId=${oneBxoId} -i bxoConfigSegUpdate bxoPriv ${site_gitServerName}
${G_myName} ${extraInfo} -p bpoId=${oneBxoId} -i bxoConfigSegDelete bxoPriv
${G_myName} ${extraInfo} -p bpoId=${oneBxoId} -i bxoConfigSegExists bxoPriv
...
_EOF_
    ...
    if [ ! -z "${bxosBase}" ] ; then
        bpoHome=$( FN_absolutePathGet ${bxosBase}/${bpoId} )
    fi
...
_EOF_
    ...
    local bpoIdsList=$( bpoAcctManage.sh -i bpoIdsList )
    local eachBxoId=""

    for eachBxoId in ${bpoIdsList} ; do
        bpoId=${eachBxoId}
        lpDo bxosBasePrep
...
_EOF_
    ...
    opDo sudo cp ${bpoHome}/$(rbxeSshBase)/id_rsa.pub ${usgHome}/.ssh/${bxoGitLabel}_${bpoId}_rsa.pub
    EH_retOnFail

    opDo sudo cp ${bpoHome}/$(rbxeSshBase)/id_rsa ${usgHome}/.ssh/${bxoGitLabel}_${bpoId}_rsa
    EH_retOnFail

    opDo sudo chown ${usg}:bisos ${usgHome}/.ssh/${bxoGitLabel}_${bpoId}_rsa.pub
    opDo sudo chown ${usg}:bisos ${usgHome}/.ssh/${bxoGitLabel}_${bpoId}_rsa

    opDo chmod 644 ${usgHome}/.ssh/${bxoGitLabel}_${bpoId}_rsa.pub
    opDo chmod 600 ${usgHome}/.ssh/${bxoGitLabel}_${bpoId}_rsa
...
_EOF_
    ...
    opDo sudo rm ${usgHome}/.ssh/${bxoGitLabel}_${bpoId}_rsa.pub
    EH_retOnFail

    opDo sudo rm ${usgHome}/.ssh/${bxoGitLabel}_${bpoId}_rsa
    EH_retOnFail
...
# BxoId=${bpoId}  bxoGitLabel=${bxoGitLabel}  bxoGitServerName=${bxoGitServerName}
Host ${bxoGitLabel}_${bpoId}
        Hostname ${bxoGitServerName}
        User git
        IdentityFile ~/.ssh/${bxoGitLabel}_${bpoId}_rsa
_EOF_
...
_EOF_
    ...
    echo "${usgSshConfigSeg_baseDir}/${bxoGitLabel}_${bpoId}.configSeg"
}
...
_EOF_
    ...
    local outFileName="${usgSshConfigSeg_baseDir}/${bxoGitLabel}_${bpoId}.configSeg"
...
_EOF_
    ...
    EH_assert bxoCentralPrep
    bxoAcctAnalyze ${bpoId}

    #bxurl="git.${cp_acctMainBaseDomain}"
    bxurl="${site_gitServerName}"

    if [ -f  ${usgHome}/.ssh/${bpoId}.dest ] ; then

        cat ${usgHome}/.ssh/${bpoId}.dest |
        while read  thisLine  ; do
            destAddr=$( echo  ${thisLine} | cut -d ':' -f 1 )
            destUser=$( echo  ${thisLine} | cut -d ':' -f 2 )

            cat  << _EOF_
ssh -vT git@${bpoId}%${destAddr}
ssh ${bpoId}%${destAddr}
ssh git@${bpoId}%${destAddr} info
_EOF_
        done
    fi
...
_EOF_
    ...
    bxurl=${bpoId}.git.bysource.org

    cat  << _EOF_
ssh -vT ${bpoId}@${bxurl} LLL
ssh ${bxurl}
ssh ${bpoId}@${bxurl} info
_EOF_

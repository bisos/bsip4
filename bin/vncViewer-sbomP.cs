...
aptPkgsList = [
    ap("tigervnc-viewer"),  #   Standalone virtual network computing server
]
...
def examples_csu() -> b.op.Outcome:
    ...
    cs.examples.menuChapter('*VNC Viewer Examples*')
    if not (resStr := b.subProc.Op(outcome=cmndOutcome, log=0).bash(
       f"""which vncviewer""").stdout):  return(b_io.eh.badOutcome(cmndOutcome))

    literal(f"which vncviewer # {resStr.strip()}")
    literal("vncviewer --version")
    literal("vncviewer --help")

    cmnd('readme', comment=" # An overview short description")

    cs.examples.menuChapter('*End-Of  VNC Viewer Examples*')
    return cmndOutcome

...
$( examplesSeperatorChapter "bpoIds List" )
bpoAcctManage.sh
...
${G_myName} ${extraInfo} -i bpoIdsList           # bxoAcctsList | cut -d ":" -f 1
$( examplesSeperatorChapter "OId Tree Info" )
${G_myName} ${extraInfo} -p bpoId="${oneBxoId}" -i ancestorsList  # NOTYET
${G_myName} ${extraInfo} -p bpoId="${oneBxoId}" -i childrenList  # NOTYET
$( examplesSeperatorChapter "File Parameters List" )
${G_myName} ${extraInfo} -p bpoId="${oneBxoId}" -i fileParamsShow
_EOF_
}
